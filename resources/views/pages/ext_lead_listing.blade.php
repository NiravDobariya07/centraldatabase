...
@section('page-title', config('app.name') . ' - Ext Lead Listing')
...
<div class="content-wrapper">
          ...
          <div class="card">
            <div class="d-flex align-items-end" >
              <div class="card-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <select id="filter_column" class="form-control">
                                    ...
                                    <option value="alt_phone">Alt Phone</option>
                                    ...
                                    <option value="jornaya_id">Jornaya ID</option>
                                    <option value="trusted_form_id">Trusted Form ID</option>
                ...
                <!-- Ext Leads Table -->
                <table class="table table-bordered table-striped" id="ext-lead-listing-table" style="width: -webkit-fill-available;">
                    <thead>
                        <tr></tr>
                    </thead>
                    <tbody>
            ...
            <form id="form-export-lead-data">
                <div class="modal-content">
                    <div class="modal-header border border-bottom-4 p-3">
                        <h5 class="modal-title">Export Ext Leads Data</h5>
                                ...
                                <div class="row mb-4">
                                    ...
                                    <div class="col-6">
                                        <div class="form-check w-auto">
                                            ...
                                            <label class="form-check-label" for="export-filtered-data"> Export Filtered Ext Leads </label>
                                        </div>
                                    </div>
                                    ...
                                    <div class="column-picker text-center">
                                        <div class="row mb-2">
                                            <div class="col-md-5">
                                                ...
                                                <select multiple id="available-columns" class="form-control column-selection-selects">
                                                    @foreach(config('export_fields.ExtLeadContact') as $field)
                                        ...
                                        <div class="row">
                                            <div class="col-6 w-auto">
                                                ...
                                                <select class="form-control" value="created_date" name="sort_by_field_name">
                                                    @foreach(config('export_fields.ExtLeadContact') as $field)
                                                        <option value="{{ $field }}" {{ ($field == 'created_date') ? "selected" : "" }} >{{ $field }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-6 w-auto">
                    ...
                    <div class="modal-footer border border-top-2 p-3">
                        ...
                        <button type="button" class="btn btn-primary" id="btn-start-ext-lead-export">Start Export</button>
                    </div>
                </div>
            <form>
        </div>
    ...
    <div class="modal fade" id="leads-listing-column-selection-model" tabindex="-1">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header border border-bottom-4 p-3">
                    <h5 class="modal-title">Ext Lead Listing – Column Customization</h5>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('.menu-item-ext-lead-listing').addClass('active');
        ...
        function initializeDataTable(selectedFields = []) {
            ...
            selectedColumns.unshift({
                ...
                render: function(data, type, row, meta) {
                    var pageInfo = $('#ext-lead-listing-table').DataTable().page.info();
            ...
            if ($.fn.DataTable.isDataTable('#ext-lead-listing-table')) {
                $('#ext-lead-listing-table').DataTable().clear().destroy();
                $('#ext-lead-listing-table thead').empty();
            }
            ...
            thead += '</tr>';
            $('#ext-lead-listing-table thead').html(thead);

            // Find the index of created_date column for ordering
            var createdDateIndex = selectedColumns.findIndex(col => col.data === 'created_date' || col.name === 'created_date');
            // If created_date column not found, default to first column (ID)
            var orderColumnIndex = createdDateIndex >= 0 ? createdDateIndex : 0;

            // Reinitialize DataTable
            table = $('#ext-lead-listing-table').DataTable({
                processing: true,
                serverSide: true,
                searching: false,
                ...
                ajax: {
                    url: "{{ route('ext-lead-listing.index') }}",
                    data: function(d) {
        ...
        $("#customFielsSaveBtn").click(function() {
            ...
            $('#preloader').show();
            $.ajax({
                url: "{{ route('save.ext-lead.field.setting') }}", // Backend route
                type: "POST",
                data: JSON.stringify({
        ...
        $("#resetCustomFielsSaveBtn").click(function() {
            $('#preloader').show();
            $.ajax({
                url: "{{ route('reset.ext-lead.field.setting') }}", // Backend route
                type: "get",
                contentType: "application/json",
                headers: {
                    "X-CSRF-TOKEN": "{{ csrf_token() }}"
        ...
        $('.filter-reset-btn').click(function () {
            ...
            // Find created_date column index for ordering
            var orderColumnIndex = 0;
            if (typeof table !== 'undefined' && table) {
                table.columns().every(function(index) {
                    if (this.dataSrc() === 'created_date') {
        ...
        $("#btn-start-ext-lead-export").on("click", function () {
            ...
            $('#preloader').show();
            $.ajax({
                url: "{{ route('ext-lead-listing.export') }}", // Laravel named route
                type: "POST",
    ...
    // Hide both fields initially
        ...
        function loadTestLeads() {
            ...
            $.ajax({
                url: '{{ route("ext-lead-listing.test-leads.get") }}',
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    if (response.success) {
        ...
        function deleteTestLeads() {
            ...
            $.ajax({
                url: '{{ route("ext-lead-listing.test-leads.delete") }}',
                type: 'POST',
                dataType: 'json',
                headers: {
