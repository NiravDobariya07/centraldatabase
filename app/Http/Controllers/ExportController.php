...
class ExportController extends Controller
{
    ...
    public function scheduleExtLeadExport(Request $request) {
        try {
            $scheduleExtLeadExportPostData = $request->input('schedule_lead_export_data', []);
            ...
            // Perform validation
            $validator = Validator::make($scheduleExtLeadExportPostData, $rules, $messages);
            ...
            $additionalData = [];
            if(!empty($scheduleExtLeadExportPostData['sort_by_field_name']) && !empty($scheduleExtLeadExportPostData['sort_by_field_order'])) {
                $additionalData['sort_by'] = [
                    'field' => $scheduleExtLeadExportPostData['sort_by_field_name'],
                    'sorting_order' => $scheduleExtLeadExportPostData['sort_by_field_order']
                ];
            }

            if (!empty($scheduleExtLeadExportPostData['export_in_batches'])) {
                $additionalData['export_in_batches'] = 1;
            }

            $scheduleExtLeadExportPayloadData = [
                'user_id'           => auth()->id(),
                'title'             => !empty(trim($scheduleExtLeadExportPostData['title'])) ? trim($scheduleExtLeadExportPostData['title']) : $defaultTitle,
                'description'       => !empty(trim($scheduleExtLeadExportPostData['description'])) ? trim($scheduleExtLeadExportPostData['description']) : null,
                'file_prefix'       => !empty(trim($scheduleExtLeadExportPostData['file_prefix'])) ? trim($scheduleExtLeadExportPostData['file_prefix']) : null,
                'export_formats'    => !empty($scheduleExtLeadExportPostData['export_formats']) ? $scheduleExtLeadExportPostData['export_formats'] : null,
                'columns'           => !empty($scheduleExtLeadExportPostData['export_columns']) ? $scheduleExtLeadExportPostData['export_columns'] : null,
                'frequency'         => !empty($scheduleExtLeadExportPostData['frequency']) ? $scheduleExtLeadExportPostData['frequency'] : null,
                'day_of_week'       => !empty($scheduleExtLeadExportPostData['day_of_week']) ? $scheduleExtLeadExportPostData['day_of_week'] : null,
                'day_of_month'      => !empty($scheduleExtLeadExportPostData['day_of_month']) ? $scheduleExtLeadExportPostData['day_of_month'] : null,
                'time'              => !empty($scheduleExtLeadExportPostData['time']) ? $scheduleExtLeadExportPostData['time'] : null,
                'additional_data'   => $additionalData,
                'runing_status'     => AppConstants::EXPORT_RUNING_STATUS['PENDING'],
                'status'            => AppConstants::EXPORT_STATUSES['ACTIVE'],
                'model_type'        => 'ExtLeadContact'
            ];

            if (!empty($scheduleExtLeadExportPostData['export_type']) && $scheduleExtLeadExportPostData['export_type'] == 'export_filtered_data') {
                $scheduleExtLeadExportPayloadData['filters'] = !empty($scheduleExtLeadExportPostData['filters']) ? $scheduleExtLeadExportPostData['filters'] : null;
            }

            // Insert into database
            $exportScheduledData = Export::create($scheduleExtLeadExportPayloadData);
            ...
            if (!empty($exportScheduledData)) {
                ...
                if ($exportScheduledData->frequency == AppConstants::EXPORT_FREQUENCY_OPTIONS['ONE_TIME']) {
                    ...
                    try {
                        // Process export synchronously
                        $this->processExtLeadExport($exportScheduledData->id);
                        ...
                        // Update status to failed
                        $exportScheduledData->update(['runing_status' => AppConstants::EXPORT_RUNING_STATUS['FAILED']]);
                        reportException($e, "Error processing instant export in scheduleExtLeadExport method");
                        return response()->json([
                            'message' => 'An error occurred while processing the export.',
                            'error'   => $e->getMessage(),
                        ], 500);
            ...
            reportException($e, "Error schedule Ext Lead Export in scheduleExtLeadExport method");
            return response()->json([
                'message' => 'An error occurred while scheduling the export.',
                'error'   => $e->getMessage(),
            ], 500); // 500 Internal Server Error
